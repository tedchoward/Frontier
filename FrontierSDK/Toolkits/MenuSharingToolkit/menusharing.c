...
static short CountMenuArray (void) {
	...
	*/
	...
	static short GetMenuHandles (void) {
		...
		*/
		...
		for (i = 0; i < ct; i++) {
			...
			if (!PushLongParam (MSglobals.clientid, 'menp', &event))
				return (false);
	...
	static pascal OSErr HandleMenuDirty (const AppleEvent *event, AppleEvent *reply, long refcon) {
	...
	static pascal OSErr HandleScriptComplete (const AppleEvent *event, AppleEvent *reply, long refcon) {
...
static Boolean InstallEventHandlers (void) {
	...
	if (AEInstallEventHandler (MSglobals.clientid, 'updm', NewAEEventHandlerProc (HandleMenuDirty), 0, false) != noErr)
		return (false);
	
	#ifdef frontierMenuSharing
	
	if (AEInstallEventHandler (MSglobals.clientid, 'done', NewAEEventHandlerProc (HandleScriptComplete), 0, false) != noErr)
		return (false);
